...
public class PacketListener_1_8 extends ChannelDuplexHandler
{
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object packet0) throws Exception {
        if (this.disconnected) {
            return;
        }
        final ExploitDetails failure = this.failure(packet0);
        if (failure == null) {
            super.channelRead(ctx, packet0);
            return;
        }
    ...
    private ExploitDetails failure(final Object packet0) {
        ...
        if (packet0 instanceof PacketPlayInWindowClick) {
                    ...
                    if (inventoryView != null) {
                        ...
                        if (packetItem != null && slot > 0 && inventoryView.getType() == InventoryType.PLAYER) {
                            try {
                                ...
                                if ((item instanceof ItemFireworks || item instanceof ItemFireworksCharge) && realItem != null && realItem.getType() != Material.valueOf("FIREWORK") && realItem.getType() != Material.valueOf("FIREWORK_CHARGE") && packetItem.getTag() != null && packetItem.getTag().toString().length() > Settings.IMP.BLOCK_FAKE_PACKETS_ONLY_WHEN_ITEMS_NBT_BIGGER_THAN) {
                                ...
                                if (item instanceof ItemBanner && realItem != null && !realItem.getType().name().contains("BANNER") && packetItem.getTag() != null && packetItem.getTag().toString().length() > Settings.IMP.BLOCK_FAKE_PACKETS_ONLY_WHEN_ITEMS_NBT_BIGGER_THAN) {
            ...
            if (packetItem2 != null) {
                ...
                if (item2 instanceof ItemFireworks || item2 instanceof ItemFireworksCharge) {
                    final org.bukkit.inventory.ItemStack realItem2 = player.getItemInHand();
                    if ((realItem2 == null || (realItem2.getType() != Material.valueOf("FIREWORK_CHARGE") && realItem2.getType() != Material.valueOf("FIREWORK") && Settings.IMP.PLACE_CHECKS.BLOCK_PLACING_FIREWORK_WHEN_NOT_HOLDING_IT)) && packetItem2.getTag() != null && packetItem2.getTag().toString().length() > Settings.IMP.BLOCK_FAKE_PACKETS_ONLY_WHEN_ITEMS_NBT_BIGGER_THAN) {
                ...
                if (item2 instanceof ItemBanner) {
                    final org.bukkit.inventory.ItemStack realItem2 = player.getItemInHand();
                    if ((realItem2 == null || (!realItem2.getType().name().contains("BANNER") && Settings.IMP.PLACE_CHECKS.BLOCK_PLACING_BANNER_WHEN_NOT_HOLDING_IT)) && packetItem2.getTag() != null && packetItem2.getTag().toString().length() > Settings.IMP.BLOCK_FAKE_PACKETS_ONLY_WHEN_ITEMS_NBT_BIGGER_THAN) {
            ...
            if (channel.equalsIgnoreCase("MC|BEdit") || channel.equalsIgnoreCase("MC|BSign") || channel.equalsIgnoreCase("minecraft:bedit") || channel.equalsIgnoreCase("minecraft:bsign")) {
                ...
                try {
                    ...
                    final ItemStack item3 = serializer.i();
                    if (item3 != null) {
                        ...
                        final ExploitDetails nbtTags4 = this.checkNbtTags(packetName, item3);
                        if (nbtTags4 != null) {
                            return nbtTags4;
                        }
                    }
            ...
            player.sendMessage("tabCompletion2: " + command);
        }
            ...
            if ((x >= Double.MAX_VALUE || y > Double.MAX_VALUE || z >= Double.MAX_VALUE) && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_MOVEMENT_V1) {
    ...
    private ExploitDetails checkNbtTags(final String packet, final ItemStack itemStack) {
        ...
        if ((item instanceof ItemWrittenBook || item instanceof ItemBookAndQuill) && Settings.IMP.BOOK.FAST_CHECK && String.valueOf(tagCompound.get("pages")).length() > Settings.IMP.BOOK.FAST_CHECK_MAX_LENGTH) {
                ...
                if (properties.hasKey("textures")) {
                    ...
                    for (int i = 0; i < textures.size(); ++i) {
                        ...
                        if (entry.hasKey("Value")) {
                            ...
                            final Matcher matcher = PacketListener_1_8.URL_MATCHER.matcher(decoded);
                            while (matcher.find()) {
