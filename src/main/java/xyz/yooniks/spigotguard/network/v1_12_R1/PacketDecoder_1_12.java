...
public class PacketDecoder_1_12 extends PacketDecoder
{
    ...
    public PacketDecoder_1_12(final PacketInjector injector, final User user) {
        super(injector);
        ...
        if (packet0.getClass().getSimpleName().equals("PacketPlayInWindowClick")) {
            ...
            try {
                ...
                if (inventoryView != null) {
                    ...
                    if (slot >= maxSlots && PacketDecoder_1_12.LIMITATION != null) {
            ...
            final PacketDataSerializer data = payload.b();
            ...
            if (p.a() >= Float.MAX_VALUE || p.b() >= Float.MAX_VALUE) {
            ...
            final Location currentVehicleMove = new Location(player.getWorld(), p2.getX(), p2.getY(), p2.getZ(), p2.getYaw(), p2.getPitch());
            if (player.getLocation().distance(currentVehicleMove) > 30.0 && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_VEHICLE_MOVEMENT_V3) {
            ...
            if (player.getLocation().distance(currentVehicleMove) > 3.0 && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_VEHICLE_MOVEMENT_V4) {
    ...
    private LimitablePacket hasReachedLimit(final String packetName) {
        if (!PacketDecoder_1_12.LIMITATION.isLimitable(packetName)) {
            return null;
        }
        final LimitablePacket limitablePacket = PacketDecoder_1_12.LIMITATION.getLimit(packetName);
                ...
                if (properties.hasKey("textures")) {
                    ...
                    for (int i = 0; i < textures.size(); ++i) {
                        ...
                        if (entry.hasKey("Value")) {
                            ...
                            final Matcher matcher = PacketDecoder_1_12.URL_MATCHER.matcher(decoded);
                            while (matcher.find()) {
